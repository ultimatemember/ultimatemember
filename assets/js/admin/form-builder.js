...
if ( typeof ( UM.formBuilder ) !== 'object' ) {
	UM.formBuilder = {};
}
	...
	section: {
		tabs: {
			setActive: function( tab_obj, $ ) {
				...
				let tabs = tab_obj.parents('.um-form-section-settings-tabs');
				tabs.find('div[data-tab]').removeClass('current');
				tab_obj.addClass('current');

				let contents = tab_obj.parents('.um-form-section-settings-tabs').siblings('.um-form-section-settings-tabs-content');
				contents.find('div[data-tab]').removeClass('current');
				contents.find('div[data-tab="' + tab + '"]').addClass('current');
			},
		},
		toggleEdit: function ( section, $ ) {
			if ( section.hasClass('um-form-section-edit-mode') ) {
				UM.formBuilder.section.hideEdit(section,$);
			} else {
				UM.formBuilder.section.showEdit(section,$);
			}
		},
		showEdit: function ( section, $ ) {
			section.addClass('um-form-section-edit-mode');
		},
		hideEdit: function ( section, $ ) {
			section.removeClass('um-form-section-edit-mode');
		},
	},
	row: {
		tabs: {
			setActive: function( tab_obj, $ ) {
				...
				let tabs = tab_obj.parents('.um-form-row-settings-tabs');
				tabs.find('div[data-tab]').removeClass('current');
				tab_obj.addClass('current');

				let contents = tab_obj.parents('.um-form-row-settings-tabs').siblings('.um-form-row-settings-tabs-content');
				contents.find('div[data-tab]').removeClass('current');
				contents.find('div[data-tab="' + tab + '"]').addClass('current');
			},
		},
		toggleEdit: function ( row, $ ) {
			if ( row.hasClass('um-form-row-edit-mode') ) {
				UM.formBuilder.row.hideEdit(row,$);
			} else {
				UM.formBuilder.row.showEdit(row,$);
			}
		},
		showEdit: function ( row, $ ) {
			row.addClass('um-form-row-edit-mode');
		},
		hideEdit: function ( row, $ ) {
			row.removeClass('um-form-row-edit-mode');
		},
	}
}
...
jQuery( function($) {
	...
	$(document.body).on('click','.um-form-section-action-edit', function(e){
		e.preventDefault();
		let section = $(this).closest('.um-form-section');
		UM.formBuilder.section.toggleEdit(section,$);
	});

	$(document.body).on('click','.um-form-section', function(e){
		if ( typeof e.target !== 'undefined' && e.target.classList.contains('um-form-section-toggle-edit') ) {
			e.preventDefault();
			let section = $(this);
			UM.formBuilder.section.toggleEdit(section,$);
			e.stopPropagation();
	...
	$(document.body).on('click','.um-form-section-settings-tabs > div[data-tab]:not(.current)', function(e){
		e.preventDefault();
		if ( $(this).hasClass('disabled') ) {
			return;
		}
		UM.formBuilder.section.tabs.setActive($(this),$);
	});




	$(document.body).on('click','.um-form-row-action-edit', function(e){
		e.preventDefault();
		let row = $(this).closest('.um-form-row');
		UM.formBuilder.row.toggleEdit(row,$);
	});

	$(document.body).on('click','.um-form-row', function(e){
		if ( typeof e.target !== 'undefined' && e.target.classList.contains('um-form-row-toggle-edit') ) {
			e.preventDefault();
			let row = $(this);
			UM.formBuilder.row.toggleEdit(row,$);
			e.stopPropagation();
	...
	$(document.body).on('click','.um-form-row-settings-tabs > div[data-tab]:not(.current)', function(e){
		e.preventDefault();
		if ( $(this).hasClass('disabled') ) {
			return;
		}
		UM.formBuilder.row.tabs.setActive($(this),$);
	});
