...
UM.forms = {
	honeypot: function () {
		...
		jQuery('input[name="' + umPasswordReset.honeypot + '"]').val('');
	}
};
...
UM.forms.recaptcha = {
	validateForm: function(e) {
		...
		grecaptcha.execute( umPasswordReset.recaptcha.site_key, {
			action: action
		}).then( function( token ) {
...
if ( typeof( umPasswordReset.recaptcha ) !== 'undefined' && 'v3' === umPasswordReset.recaptcha.version ) {
	grecaptcha.ready( function() {
		jQuery('.g-recaptcha').closest('form').on( 'submit', UM.forms.recaptcha.validateForm );
	});
} else if ( typeof( umPasswordReset.recaptcha ) !== 'undefined' && 'v2' === umPasswordReset.recaptcha.version ) {
	if ( 'invisible' === umPasswordReset.recaptcha.size ) {
		var umPasswordResetRecaptchaCallback = function() {
			grecaptcha.render( 'um-new-password', {
				'sitekey': umPasswordReset.recaptcha.site_key,
