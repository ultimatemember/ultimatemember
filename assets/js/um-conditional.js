jQuery(document).ready( function (){
    jQuery(".um-form").each(function() {
        ...
        function um_field_apply_action($dom, condition, is_true) {
            var child_dom = $form.find('.um-field[data-key="' + condition.owner + '"]');
        ...
        function um_field_hide_siblings() {
            ...
            jQuery.each(um_field_conditions, function (index, conditions) {
                if ($form.find('.um-field[data-key="' + index + '"]:hidden').length >= 1 ||
                    $form.find('.um-field[data-key="' + index + '"]').css('display') === 'none'
                ) {
                    jQuery.each(conditions, function (key, condition) {
                        $form.find('.um-field[data-key="' + condition.owner + '"]').hide();
                    });
                }
        ...
        $form.on('change', '.um-field select, .um-field input[type=radio], .um-field input[type=checkbox]', function () {
            var me = jQuery(this);
            um_apply_conditions(me, false);
        });

        $form.on('input change', '.um-field input[type=text]', function () {
            var me = jQuery(this);
            um_apply_conditions(me, false);
        });

        $form.on('input change', '.um-field input[type=password]', function () {
            var me = jQuery(this);
            um_apply_conditions(me, false);
        });

        $form.on('um_fields_change', function () {
        ...
        function um_init_field_conditions() {
            ...
            $form.find('.um-field[data-key]').each(function () {
            ...
            $form.find('.um-field[data-key]:visible').each(function () {
