...
// modules/recaptcha/assets/js/login.js
/**
 * reCAPTCHA v3
...
UM.forms.recaptcha = {
	validateForm: function(e) {
		...
		grecaptcha.execute( umLogin.recaptcha.site_key, {
			action: action
		}).then( function( token ) {
...
if ( typeof( umLogin.recaptcha ) !== 'undefined' && 'v3' === umLogin.recaptcha.version ) {
	grecaptcha.ready( function() {
		jQuery('.g-recaptcha').closest('form').on( 'submit', UM.forms.recaptcha.validateForm );
	});
} else if ( typeof( umLogin.recaptcha ) !== 'undefined' && 'v2' === umLogin.recaptcha.version ) {
	if ( 'invisible' === umLogin.recaptcha.size ) {
		var umLoginRecaptchaCallback = function() {
			grecaptcha.render( 'wp-submit', {
				'sitekey': umLogin.recaptcha.site_key,
		...
		jQuery(document).ready( function() {
			jQuery('#wp-submit').addClass('um-has-recaptcha');
		});
	} else {
		var umLoginRecaptchaCallback = function() {
