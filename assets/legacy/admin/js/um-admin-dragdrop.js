...
function UM_update_rows() {
	...
	jQuery('.um-admin-drag-row').each(function(){
		...
		row = jQuery(this);
		if ( c != 1 ) {
			row.find('.um-admin-drag-row-icons').append( '<a href="#" class="um-admin-tipsy-n" title="Delete Row" data-remove_element="um-admin-drag-row"><i class="far fa-trash-alt"></i></a>' );
		}
	});
}
...
function UM_update_subrows(){
	...
	jQuery('.um-admin-drag-row').each(function(){
		...
		row = jQuery(this);
		if ( c != 1 ) {
			row.find('.um-admin-drag-rowsub-icons').append('<a href="#" class="um-admin-tipsy-n" title="Delete Row" data-remove_element="um-admin-drag-rowsub"><i class="far fa-trash-alt"></i></a>');
		}
		});
	});
}
...
function UM_Add_Icon(){
	...
	var add_icon_html = '<a href="#" class="um-admin-drag-add-field um-admin-tipsy-n" title="Add Field" data-modal="UM_fields" data-modal-size="normal" data-dynamic-content="um_admin_show_fields" data-arg2="'+jQuery('.um-admin-drag-ajax').data('form_id')+'" data-arg1=""><i class="fas fa-plus"></i></a>';
