...
if ( ! class_exists( 'um\core\Password' ) ) {
	...
	class Password {
		...
		public function um_change_password_errors_hook( $args ) {
			...
			if( isset( $args['user_password'] ) && empty( $args['user_password'] ) && UM()->options()->get('change_password_request_limit') && is_user_logged_in()  ){
				...
				if( ! $last_request ){
					set_transient( $transient_id, current_time('mysql'), $request_limit_time );
				}else{
