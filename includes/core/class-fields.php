	...
	class Fields {
		...
		public function add_hidden_field( $field ) {
			...
			foreach ( $fields as $key => $data ) {
				$output .= $this->edit_field( $key, $data, false, array( 'hidden' => true ) );
			}

			if ( ! empty( $output ) ) {
				echo '<div style="display: none !important;">' . wp_kses( $output, UM()->get_allowed_html( 'templates' ) ) . '</div>';
			}
		}
		...
		public function edit_field( $key, $data, $rule = false, $args = array() ) {
			...
			switch ( $type ) {
				...
				/* Single Image Upload */
				case 'image':
					if ( ( in_array( $key, array( 'profile_photo', 'cover_photo' ), true ) && ! empty( $args['hidden'] ) ) || ! in_array( $key, array( 'profile_photo', 'cover_photo' ), true ) || UM()->is_new_ui() ) {
						// User Profile Photo/Cover Photo fields aren't supported in old UI on the form.
						...
						if ( is_array( $field_value ) ) {
							$field_value = ! empty( $field_value['filename'] ) ? $field_value['filename'] : 'empty_file';
						}
				...
				case 'file':
					...
					if ( is_array( $file_field_value ) ) {
						$file_field_value = ! empty( $file_field_value['filename'] ) ? $file_field_value['filename'] : 'empty_file';
					}
