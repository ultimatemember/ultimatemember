	...
	class Fields {
		...
		public function edit_field( $key, $data, $rule = false, $args = array() ) {
			...
			switch ( $type ) {
				...
				/* Single Image Upload */
				case 'image':
					$output .= '<div ' . $this->get_atts( $key, $classes, $conditional, $data ) . ' data-mode="' . esc_attr( $this->set_mode ) . '" data-guest_token="' . esc_attr( UM()->common()->guest()->get_guest_token() ) . '" data-remove_nonce="' . esc_attr( wp_create_nonce( 'um_remove_file' . $this->set_mode ) ) . '" data-upload-label="' . ( ! empty( $data['button_text'] ) ? esc_attr( $data['button_text'] ) : esc_attr__( 'Upload', 'ultimate-member' ) ) . '">';
