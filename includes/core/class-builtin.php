...
if ( ! class_exists( 'um\core\Builtin' ) ) {
	...
	class Builtin {
		...
		function all_user_fields( $exclude_types = null, $show_all = false ) {
			...
			$exclude_array = $this->fields_dropdown;
			if ( $exclude_types && is_array( $exclude_types ) ) {
				$exclude_array = array_merge( $exclude_array, $exclude_types );
			}
			...
			foreach ( $all as $k => $arr ) {
				...
				if ( isset( $arr['type'] ) && in_array( $arr['type'], $exclude_array, true ) ) {
					unset( $all[ $k ] );
					continue;
				}

				if ( ( isset( $arr['account_only'] ) || isset( $arr['private_use'] ) ) && ! $show_all ) {
					unset( $all[ $k ] );
					continue;
				}
			...
			return UM()->fields()->array_sort_by_column( $all, 'title' );
		}
