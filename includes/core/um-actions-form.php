...
function um_submit_form_errors_hook_( $args ) {
	...
	if ( ! empty( $args['um_visible_fields'] ) ) {
		$visible_fields = explode( ',', $args['um_visible_fields'] );
	}
	...
	foreach ( $fields as $key => $array ) {
		...
		/* Required visible field is empty */
		if ( ! empty( $array['required'] )
				&& ( empty( $array['conditions'] ) || empty( $visible_fields ) || in_array( $key, $visible_fields ) )
				&& ( empty( $args[$key] ) || $args[$key] == 'empty_file' ) ) {

			UM()->form()->add_error( $key, empty( $array['label'] ) ? __( 'This field is required', 'ultimate-member' ) : sprintf( __( '%s is required', 'ultimate-member' ), $array['label'] )  );
		}
