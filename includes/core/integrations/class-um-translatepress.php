...
class UM_TranslatePress implements UM_Multilingual {
	...
	public function emails_column_content( $item ) {
		...
		if ( $this->is_active() ) {
			foreach ( $this->languages_list as $language_code ) {
				...
				$html .= $this->emails_column_content_item( $item['key'], $language_code );
			}
		}
	...
	public function get_current_locale() {
		...
		if ( isset( $_GET['lang'] ) ) {
			$current_code = sanitize_key( $_GET['lang'] );
		}
		...
		if ( !empty( $current_code ) ) {
			$slugs = $this->tpr_settings->get_setting( 'url-slugs' );
			if ( strlen( $current_code ) === 2 ) {
				$current_locale = current( array_keys( $slugs, $current_code ) );
			}
		}

		return empty( $current_locale ) ? get_locale() : $current_locale;
	}
	...
	public function get_languages_codes( $current_code = false ) {
		...
		if ( empty( $current_code ) ) {
			...
			if ( strlen( $current_code ) === 2 ) {
				$current = current( array_keys( $slugs, $current_code ) );
			}
		}
