...
if ( ! class_exists( 'um\frontend\Forms' ) ) {
	...
	class Forms {
		...
		public function display( $echo = true ) {
			...
			if ( ! $move_form_tag ) {
				echo wp_kses( '<form action="' . esc_attr( $action ) . '" method="' . esc_attr( $method ) . '" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" class="um-form" ' . $data_attr . '>', UM()->get_allowed_html( 'templates' ) );
			}
		...
		public function render_datepicker( $field_data ) {
			...
			$class      = ! empty( $field_data['class'] ) ? $field_data['class'] : '';
			$class     .= ! empty( $field_data['size'] ) ? 'um-' . $field_data['size'] . '-field' : 'um-long-field';
			$class_attr = ' class="um-forms-field um-datepicker ' . esc_attr( $class ) . '" ';
			...
			$html = "<input type=\"text\" $id_attr $class_attr $data_attr $value_attr $placeholder_attr /><input type=\"hidden\" class=\"um-datepicker-default-format\" $name_attr $hidden_value_attr />";
