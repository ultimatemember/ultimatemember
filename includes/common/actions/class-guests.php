...
if ( ! class_exists( 'um\common\actions\Guests' ) ) {
	...
	class Guests {
		...
		public function flush_guest_data() {
			...
			$wpdb->query(
				$wpdb->prepare(
					"DELETE FROM {$wpdb->prefix}um_guest_tokens
					WHERE created_at < NOW() - INTERVAL %d DAY",
					$tokens_ttl_days
				)
			);
			...
			$wpdb->query(
				$wpdb->prepare(
					"DELETE FROM {$wpdb->prefix}um_guest_download_attempts
			...
			$wpdb->query( "ALTER TABLE {$wpdb->prefix}um_guest_download_attempts AUTO_INCREMENT = 1" );
		}
	}
}
