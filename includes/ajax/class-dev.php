...
class Dev {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_nopriv_um_get_link_snippet', array( $this, 'get_link_snippet' ) );
		add_action( 'wp_ajax_um_get_link_snippet', array( $this, 'get_link_snippet' ) );
	}
...
<?php echo '?>'; ?>
	...
	public function get_link_snippet() {
		if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'get_link' ) ) {
			wp_send_json_error( __('Wrong nonce', 'ultimate-member') );
		}
		...
		$link = UM()->frontend()::layouts()::link( $args['content'], $args );
		ob_start();
		if ( is_null( $args['icon_position'] ) ) {
		?>
<code style="float:left;width: 100%;overflow:auto;border:1px solid #aaa; background: #eee;">
...
<?php echo '?>'; ?>
		...
		$content = UM()->ajax()->esc_html_spaces( $link ) . $snippet;
