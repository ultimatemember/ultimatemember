...
/**
 * Class Directory_Meta
 *
 * @package um\ajax
 */
class Directory_Meta extends Directory {
	...
	public function ajax_get_members_inner() {
		...
		} elseif ( ! empty( $cover_photo_where ) || ! empty( $profile_photo_where ) ) {
			if ( ! $this->general_meta_joined ) {
				$this->joins[] = "LEFT JOIN {$wpdb->prefix}um_metadata umm_general ON umm_general.user_id = u.ID";
		...
		if ( ! empty( $_POST['search'] ) ) {
			$search_line = $this->prepare_search( $_POST['search'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- make this inside the function.
