...
class Updater {
	 ...
	 *
	 * Please note that these functions are invoked when Ultimate Member is updated from a previous version,
	 * but NOT when Ultimate Member is newly installed.
	 *
	 * @var array[]
	 */
	...
	private static function prepare_packages() {
		$packages = array(
			'core' => array(
				'list'       => self::$db_updates,
				'path'       => UM_PATH . '/includes/updates/',
				'version'    => UM_VERSION,
				'db_version' => get_option( 'um_last_version_upgrade', '1.3.88' ),
			),
		);
		return apply_filters( 'um_updater_db_packages', $packages );
	}

	private static function update_required() {
		$packages = self::prepare_packages();

		$required_callbacks = array();
		foreach ( $packages as $plugin => $data ) {
			...
			foreach ( $data['list'] as $version => $callbacks ) {
				if ( version_compare( $data['db_version'], $version, '<' ) && version_compare( $version, $data['version'], '<=' ) ) {
					$required_callbacks[ $plugin ][ $version ] = $callbacks;
				}
			}
		}

		return ! empty( $required_callbacks );
	}
	...
	public function __construct() {
		...
		if ( ! empty( self::update_required() ) ) {
			add_action( 'admin_menu', array( $this, 'admin_menu' ), 0 );
			// add_action( 'wp_loaded', array( $this, 'initialize_upgrade_packages' ), 0 );
		}

		add_action( 'admin_init', array( $this, 'prepare_update_messages' ) );
	}

	public function prepare_update_messages() {
		$plugin_slugs = array_merge(
			array( UM_PLUGIN ),
			array_map(
				static function ( $value ) {
					return 'um-' . $value . '/um-' . $value . '.php';
				},
				UM()->config()::$extensions
			)
		);

		foreach ( $plugin_slugs as $slug ) {
			add_action( 'in_plugin_update_message-' . $slug, array( $this, 'in_plugin_update_message' ) );
		}
	}
	...
	public function in_plugin_update_message( $args ) {
		...
		if ( isset( $args['new_version'], $args['Version'] ) ) {
			$old_version_array = explode( '.', $args['Version'] );
			$new_version_array = explode( '.', $args['new_version'] );

			if ( $old_version_array[0] < $new_version_array[0] || $old_version_array[1] < $new_version_array[1] ) {
		...
		$classes = array(
			'um-plugin-upgrade-notice',
			$args['slug'] . '-upgrade-notice',
		);
