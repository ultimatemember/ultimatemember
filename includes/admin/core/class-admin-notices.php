		...
		function install_predefined_page_notice() {
			...
			if ( $user_page_id = um_get_predefined_page_id( 'user' ) ) {
				...
				if ( isset( $test->post_parent ) && $test->post_parent > 0 ) {
					$this->add_notice( 'wrong_user_page', array(
						'class'   => 'updated',
						'message' => '<p><strong>' . __( 'Ultimate Member Setup Error:', 'ultimate-member' ) . '</strong> ' . __( 'User page can not be a child page.', 'ultimate-member' ) . '</p>',
					), 25 );
				}
			}
			...
			if ( $account_page_id = um_get_predefined_page_id( 'account' ) ) {
				...
				if ( isset( $test->post_parent ) && $test->post_parent > 0 ) {
					$this->add_notice( 'wrong_account_page', array(
						'class'   => 'updated',
						'message' => '<p><strong>' . __( 'Ultimate Member Setup Error:', 'ultimate-member' ) . '</strong> ' . __( 'Account page can not be a child page.', 'ultimate-member' ) . '</p>'
					), 30 );
				}
			}
			...
			if ( um_get_predefined_page_id( 'user' ) === um_get_predefined_page_id( 'account' ) ) {
				$this->add_notice( 'wrong_account_user_page', array(
					'class'   => 'error',
					'message' => '<p><strong>' . __( 'Ultimate Member Setup Error:', 'ultimate-member' ) . '</strong> ' . __( 'The Account page and User page must be separate pages.', 'ultimate-member' ) . ' <a href="admin.php?page=um_options">' . __( 'Go to pages settings.', 'ultimate-member' ) . '</a></p>'
				), 30 );
			}
			...
			if ( function_exists( 'wc_get_page_id' ) && wc_get_page_id( 'myaccount' ) === um_get_predefined_page_id( 'account' ) ) {
				$this->add_notice( 'wrong_account_my_account_page', array(
					'class'   => 'error',
					'message' => '<p><strong>' . __( 'Ultimate Member Setup Error:', 'ultimate-member' ) . '</strong> ' . __( 'The Account page and WooCommerce "My account" page should be separate pages.', 'ultimate-member' ) . ' <a href="admin.php?page=um_options">' . __( 'Go to pages settings.', 'ultimate-member' ) . '</a></p>'
				), 30 );
			}

			if ( um_get_predefined_page_id( 'logout' ) === (int) get_option('page_on_front') ) {
				$this->add_notice( 'wrong_home_logout_page', array(
					'class'   => 'error',
					'message' => '<p><strong>' . __( 'Ultimate Member Setup Error:', 'ultimate-member' ) . '</strong> ' . __( 'The Home page must not be chosen as the Logout page.', 'ultimate-member' ) . ' <a href="admin.php?page=um_options">' . __( 'Go to pages settings.', 'ultimate-member' ) . '</a></p>'
				), 30 );
			}
		}
