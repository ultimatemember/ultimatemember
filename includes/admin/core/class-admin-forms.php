...
if ( ! class_exists( 'um\admin\core\Admin_Forms' ) ) {
	...
	class Admin_Forms {
		...
		public function render_field_by_hook( $data ) {
			 ...
			 * @param {object} $form       Backend form class (\um\admin\core\Forms) instance.
			 *
			 * @return {string} Rendered content.
			 *
			 * @example <caption>Generate custom content for a field with applied `info_block` field-type</caption>
			 * add_filter( 'um_render_field_type_info_block', 'my_render_field_type', 10, 4 );
		...
		public function render_email( $field_data ) {
			...
			$html = "<input type=\"email\" $id_attr $class_attr $name_attr $data_attr $value_attr $placeholder_attr " . disabled( ! empty( $field_data['disabled'] ), true, false ) . ' />';
			return $html;
		}
		...
		public function render_inline_texts( $field_data ) {
			...
			return vsprintf( $field_data['mask'], $fields );
		}
		...
		public function render_textarea( $field_data ) {
			...
			$html = "<textarea $id_attr $class_attr $name_attr $data_attr $rows " . disabled( ! empty( $field_data['disabled'] ), true, false ) . '>' . esc_textarea( $value ) . '</textarea>';
			return $html;
		}
...
//			$html = "<input type=\"hidden\" $id_attr_hidden $name_attr value=\"0\" />{$field_html}";
		...
		public function render_select( $field_data ) {
			...
			$data_attr = '';
			foreach ( $data as $key => $value ) {
				$data_attr .= ' data-' . esc_html( $key ) . '="' . esc_attr( $value ) . '" ';
			}
					...
					if ( is_array( $option ) && ! empty( $option['options'] ) ) {
						...
						foreach ( $option['options'] as $sub_key => $sub_option ) {
							if ( ! empty( $field_data['multi'] ) ) {
								...
								$options .= '<option value="' . $sub_key . '" ' . selected( in_array( (string) $sub_key, $value, true ), true, false ) . ' ' . disabled( in_array( $sub_key, $disabled_options, true ), true, false ) . '>' . esc_html( $sub_option ) . '</option>';
							} else {
								$options .= '<option value="' . $sub_key . '" ' . selected( (string) $sub_key === (string) $value, true, false ) . ' ' . disabled( in_array( $sub_key, $disabled_options, true ), true, false ) . '>' . esc_html( $sub_option ) . '</option>';
							}
						}
						$options .= '</optgroup>';
					} else {
						if ( ! empty( $field_data['multi'] ) ) {
							...
							$options .= '<option value="' . $key . '" ' . selected( in_array( (string) $key, $value, true ), true, false ) . ' ' . disabled( in_array( $key, $disabled_options, true ), true, false ) . '>' . esc_html( $option ) . '</option>';
						} else {
							$options .= '<option value="' . $key . '" ' . selected( (string) $key === (string) $value, true, false ) . ' ' . disabled( in_array( $key, $disabled_options, true ), true, false ) . '>' . esc_html( $option ) . '</option>';
						}
					}
				}
			}
		...
		public function render_page_select( $field_data ) {
			...
			if ( ! empty( $field_data['options'] ) ) {
				foreach ( $field_data['options'] as $key => $option ) {
					if ( ! empty( $field_data['multi'] ) ) {
						...
						$options .= '<option value="' . esc_attr( $key ) . '" ' . selected( (string) $key === (string) $value, true, false ) . '>' . esc_html( $option ) . '</option>';
					}
				}
			}
			...
			if ( ! um_get_predefined_page_id( $slug ) || 'publish' !== get_post_status( um_get_predefined_page_id( $slug ) ) ) {
				$install_page_link = add_query_arg(
					array(
						'um_adm_action' => 'install_predefined_page',
						'um_page_slug'  => $slug,
					)
				);

				$button = '&nbsp;<a href="' . esc_url( $install_page_link ) . '" class="button button-primary">' . esc_html__( 'Create Default', 'ultimate-member' ) . '</a>';
			}
		...
		public function get_field_value( $field_data, $i = '' ) {
			...
			if ( isset( $field_data[ $default_key ] ) ) {
				$default = $field_data[ $default_key ];
			}

			if ( 'checkbox' === $field_data['type'] || 'multi_checkbox' === $field_data['type'] ) {
				$value = ( isset( $field_data[ $value_key ] ) && '' !== $field_data[ $value_key ] ) ? $field_data[ $value_key ] : $default;
			} else {
				$value = isset( $field_data[ $value_key ] ) ? $field_data[ $value_key ] : $default;
			}
