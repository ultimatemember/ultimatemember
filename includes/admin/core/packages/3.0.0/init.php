...
<script type="text/javascript">
	jQuery( document ).ready( function() {
		...
		jQuery.ajax({
			...
			success: function( response ) {
				if ( typeof response.data.count != 'undefined' ) {
					...
					setTimeout( function () {
						um_update_metadata_per_user300();
					}, um_request_throttle );
				} else {
					um_wrong_ajax();
				}
		...
		function um_update_metadata_per_user300() {
			if ( current_page <= users_pages ) {
				jQuery.ajax({
					...
					dataType: 'json',
					data: {
						action: 'um_metadata_per_user300',
						page: current_page,
						nonce: um_admin_scripts.nonce
					},
					success: function( response ) {
