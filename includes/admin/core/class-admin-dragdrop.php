	...
	class Admin_DragDrop {
		...
		public function update_order() {
			...
			if ( empty( $_POST['form_id'] ) ) {
				wp_send_json_error( __( 'Wrong input data', 'ultimate-member' ) );
			}
			...
			foreach ( $_POST as $key => $value ) {
				...
				if ( 0 === strpos( $key, '_um_row_' ) ) {
					...
					if ( isset( $_POST[ '_um_rowcols_' . $row_id . '_cols' ] )
						&& isset( $_POST[ '_um_rowsub_' . $row_id . '_rows' ] )
						&& isset( $_POST[ '_um_roworigin_' . $row_id . '_val' ] )
							) {

						$cols = sanitize_text_field( wp_unslash( $_POST[ '_um_rowcols_' . $row_id . '_cols' ] ) );
						if ( ! strstr( $cols, ':' ) ) {
							$cols = absint( $cols );
						}
