...
if ( ! class_exists( 'um\admin\core\Admin_Metabox' ) ) {
	...
	class Admin_Metabox {
		...
		public function add_metabox() {
			...
			if ( isset( $current_screen ) && ! empty( $post_types[ $current_screen->id ] ) ) {
		...
		/**
		 * Add post/page metabox Ultimate Member: Content Restriction
		 */
		public function add_metabox_restrict_content() {
			...
			add_meta_box(
				...
				array( &$this, 'load_metabox_restrict' ),
				$current_screen->id,
				'normal',
				'default'
			);
		 ...
		 * Add role metabox
		 *
		 * @param string $meta  Meta Box Key.
		 */
		...
		public function add_taxonomy_metabox() {
			...
			foreach ( $all_taxonomies as $key => $taxonomy ) {
				if ( in_array( $key, $exclude_taxonomies, true ) || empty( $tax_types[ $key ] ) ) {
					continue;
				}
		 ...
		 * @return mixed
		 */
		public function get_custom_post_meta( $id ) {
		 ...
		 *
		 * @param  bool $hide  Show Metabox.
		 *
		 * @return bool
		 */
		public function hide_metabox_restrict_content_shop( $hide ) {
			if (
		 ...
		 * @param array   $box     Metabox data.
		 */
		public function load_metabox_role( $object, $box ) {
			global $post;
		 ...
		 * @param array   $box     Metabox data.
		 */
		public function load_metabox_restrict( $object, $box ) {
		...
		public function load_modal_content() {
			...
			if ( isset( $screen ) && strstr( $screen->id, 'um_form' ) ) {
			...
			if ( ! isset( $this->is_loaded ) && isset( $screen ) && strstr( $screen->id, 'um_form' ) ) {
		 ...
		 */
		public function save_attachment_metabox_restrict_content( $post_id ) {
			// validate nonce.
			if (
		...
		public function save_metabox_directory( $post_id, $post ) {
			...
			$wpdb->update(
				$wpdb->posts,
				array( 'post_title' => sanitize_text_field( wp_unslash( $_POST['post_title'] ) ) ),
				$where
			);
		 ...
		 */
		public function um_category_access_fields_create( $taxonomy = '' ) {
			/**
			 * UM hook
			 *
			 * @type filter
		 ...
		 */
		public function um_category_access_fields_edit( $term ) {
			$term_id = $term->term_id;

			$data = get_term_meta( $term_id, 'um_content_restriction', true );
		 ...
		 * @return int
		 */
		public function um_category_access_fields_save( $term_id ) {
		 ...
		 * @return bool
		 */
		public function validation_types_continue_loop( $break, $key, $form_id, $field_array ) {

			// show unique username validation only for user_login field.
			if (
				isset( $field_array['metakey'] )
				&& 'user_login' === $field_array['metakey']
				&& 'unique_username' !== $key
			) {
				return false;
			}
		 ...
		 * @param int    $form_id     Form ID.
		 * @param array  $field_args  An array of arguments.
		 */
		public function field_input( $attribute, $form_id = null, $field_args = array() ) {
			...
			switch ( $attribute ) {
				...
				case '_visibility':
					...
					<p>
						...
						<select name="_visibility" id="_visibility" style="width: 100%">
							<option value="all" <?php selected( 'all', $this->edit_mode_value ); ?>><?php esc_html_e( 'View everywhere', 'ultimate-member' ); ?></option>
							<option value="edit" <?php selected( 'edit', $this->edit_mode_value ); ?>><?php esc_html_e( 'Edit mode only', 'ultimate-member' ); ?></option>
							<option value="view" <?php selected( 'view', $this->edit_mode_value ); ?>><?php esc_html_e( 'View mode only', 'ultimate-member' ); ?></option>
						</select>
					</p>
				...
				case '_validate':
					...
					<p>
						<label for="_validate"><?php esc_html_e( 'Validate', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Does this field require a special validation', 'ultimate-member' ) ); ?></label>
				...
				case '_borderstyle':
					...
					<p>
						<label for="_borderstyle"><?php esc_html_e( 'Style', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Choose the border style', 'ultimate-member' ) ); ?></label>
				...
				case '_heading':
					...
					<p>
						<label for="_heading"><?php esc_html_e( 'Enable Row Heading', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Whether to enable a heading for this row', 'ultimate-member' ) ); ?></label>
				...
				case '_force_confirm_pass':
					...
					<p>
						<label for="_force_confirm_pass"><?php esc_html_e( 'Automatically add a confirm password field?', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Turn on to add a confirm password field. If turned on the confirm password field will only show on register forms and not on login forms.', 'ultimate-member' ) ); ?></label>
				...
				case '_style':
					...
					<p>
						<label for="_style"><?php esc_html_e( 'Style', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'This is the line-style of divider', 'ultimate-member' ) ); ?></label>
				...
				case '_pretty_format':
					...
					<p>
						<label for="_pretty_format"><?php esc_html_e( 'Displayed Date Format', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Whether you wish to show the date in full or only show the years e.g. 25 Years', 'ultimate-member' ) ); ?></label>
				...
				case '_years_x':
					...
					<p class="_years">
						<label for="_years_x"><?php esc_html_e( 'Years Selection', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'This decides which years should be shown relative to today date', 'ultimate-member' ) ); ?></label>
						<select name="_years_x" id="_years_x" style="width: 100%">
							<option value="equal"  <?php selected( 'equal', $this->edit_mode_value ); ?>><?php esc_html_e( 'Equal years before / after today', 'ultimate-member' ); ?></option>
							<option value="past" <?php selected( 'past', $this->edit_mode_value ); ?>><?php esc_html_e( 'Past years only', 'ultimate-member' ); ?></option>
							<option value="future" <?php selected( 'future', $this->edit_mode_value ); ?>><?php esc_html_e( 'Future years only', 'ultimate-member' ); ?></option>
						</select>
					</p>
				...
				case '_range':
					...
					<p>
						<label for="_range"><?php esc_html_e( 'Set Date Range', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Whether to show a specific number of years or specify a date range to be available for the date picker.', 'ultimate-member' ) ); ?></label>
						<select name="_range" id="_range" class="um-adm-conditional" data-cond1='years' data-cond1-show='_years' data-cond2="date_range" data-cond2-show="_date_range" style="width: 100%">
							<option value="years" <?php selected( 'years', $this->edit_mode_value ); ?>><?php esc_html_e( 'Fixed Number of Years', 'ultimate-member' ); ?></option>
							<option value="date_range" <?php selected( 'date_range', $this->edit_mode_value ); ?>><?php esc_html_e( 'Specific Date Range', 'ultimate-member' ); ?></option>
						</select>
					</p>
				...
				case '_content':
					if ( $this->set_field_type == 'shortcode' ) {
						...
						<div class="um-admin-editor-h"><label><?php esc_html_e( 'Content Editor', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Edit the content of this field here', 'ultimate-member' ) ); ?></label></div>
				...
				case '_is_multi':
					...
					<p>
						<label for="_is_multi"><?php esc_html_e( 'Allow multiple selections', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Enable/disable multiple selections for this field', 'ultimate-member' ) ); ?></label>
				...
				case '_html':
					...
					<p>
						<label for="_html"><?php esc_html_e( 'Does this textarea accept HTML?', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Turn on/off HTML tags for this textarea', 'ultimate-member' ) ); ?></label>
				...
				case '_metakey':
					...
					<?php } else { ?>
						...
						<p>
							<label for="_metakey"><?php esc_html_e( 'Meta Key', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'A meta key is required to store the entered info in this field in the database. The meta key should be unique to this field and be written in lowercase with an underscore ( _ ) separating words e.g country_list or job_title', 'ultimate-member' ) ); ?></label>
				...
				<?php } elseif ( 'rating' === $this->set_field_type ) { ?>
					...
					<p>
						<label for="_default"><?php esc_html_e( 'Default Rating', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'If you wish the rating field to be prefilled with a number of stars, enter it here.', 'ultimate-member' ) ); ?></label>
				...
				<?php } else { ?>
					...
					<p>
						<label for="_default"><?php esc_html_e( 'Default Value', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'This option allows you to pre-fill the field with a default value prior to the user entering a value in the field. Leave blank to have no default value', 'ultimate-member' ) ); ?></label>
						<input type="text" name="_default" id="_default" value="<?php echo esc_attr( $this->edit_mode_value ); ?>" />
					</p>
				...
				case '_label':
					...
					<p>
						<label for="_label"><?php esc_html_e( 'Label', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'The field label is the text that appears above the field on your front-end form. Leave blank to not show a label above field.', 'ultimate-member' ) ); ?></label>
				...
				case '_label_confirm_pass':
					...
					<p>
						<label for="_label_confirm_pass"><?php esc_html_e( 'Confirm password field label', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'This label is the text that appears above the confirm password field. Leave blank to show default label.', 'ultimate-member' ) ); ?></label>
						<input type="text" name="_label_confirm_pass" id="_label_confirm_pass" value="<?php echo esc_attr( htmlspecialchars( $this->edit_mode_value, ENT_QUOTES ) ); ?>" />
					</p>
				...
				case '_placeholder':
					...
					<p>
						<label for="_placeholder"><?php esc_html_e( 'Placeholder', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'This is the text that appears within the field e.g please enter your email address. Leave blank to not show any placeholder text.', 'ultimate-member' ) ); ?></label>
				...
				case '_public':
					...
					<p>
						<label for="_public"><?php esc_html_e( 'Privacy', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Field privacy allows you to select who can view this field on the front-end. The site admin can view all fields regardless of the option set here.', 'ultimate-member' ) ); ?></label>
				...
				case '_roles':
					...
					<p class="_roles">
						<label for="_roles"><?php esc_html_e( 'Select member roles', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Select the member roles that can view this field on the front-end.', 'ultimate-member' ) ); ?></label>
				...
				case '_required':
					if ( 'password' === $this->set_field_type ) {
						$def_required = 1;
					} else {
						$def_required = 0;
					}
					...
					<div class="um-admin-tri">
						...
						<p>
							<label for="_required"><?php esc_html_e( 'Is this field required?', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'This option allows you to set whether the field must be filled in before the form can be processed.', 'ultimate-member' ) ); ?></label>
				...
				case '_editable':
					...
					<div class="um-admin-tri">
						...
						<p>
							<label for="_editable"><?php esc_html_e( 'Can user edit this field?', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'This option allows you to set whether or not the user can edit the information in this field. The site admin can edit all fields regardless of the option set here.', 'ultimate-member' ) ); ?></label>
				...
				case '_number':
					...
					<p>
						<label for="_number"><?php esc_html_e( 'Rating System', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Choose whether you want a 5-stars or 10-stars ratings based here.', 'ultimate-member' ) ); ?></label>
						<select name="_number" id="_number" style="width: 100%">
							<option value="5" <?php selected( 5, $this->edit_mode_value ); ?>><?php esc_html_e( '5  stars rating system', 'ultimate-member' ); ?></option>
							<option value="10" <?php selected( 10, $this->edit_mode_value ); ?>><?php esc_html_e( '10 stars rating system', 'ultimate-member' ); ?></option>
						</select>
					</p>
				...
				case '_custom_dropdown_options_source':
					...
					<p>
						<label for="_custom_dropdown_options_source"><?php esc_html_e( 'Choices Callback', 'ultimate-member' ); ?> <?php UM()->tooltip( __( 'Add a callback source to retrieve choices.', 'ultimate-member' ) ); ?></label>
				...
				case '_parent_dropdown_relationship':
					...
					<p>
						<label for="_parent_dropdown_relationship"><?php esc_html_e( 'Parent Option', 'ultimate-member' ); ?><?php UM()->tooltip( __( 'Dynamically populates the option based from selected parent option.', 'ultimate-member' ) ); ?></label>
						<select name="_parent_dropdown_relationship" id="_parent_dropdown_relationship" style="width: 100%">
							...
							if ( UM()->builtin()->custom_fields ) {
								foreach ( UM()->builtin()->custom_fields as $field_key => $array ) {
									if (
										in_array( $array['type'], array( 'select' ), true )
										&& ( ! isset( $field_args['metakey'] ) || $field_args['metakey'] !== $array['metakey'] )
										&& isset( $array['title'] )
									) {
										?>
