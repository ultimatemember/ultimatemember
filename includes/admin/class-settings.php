...
class Settings {
	 ...
	 * @var array
	 */
	public $settings_structure = array();
	...
	public function __construct() {
		...
		add_action( 'um_settings', array( &$this, 'register_settings' ) );
	...
	public function add_tab( $title, $slug, $callback = null ) {
		if ( array_key_exists( $slug, $this->settings_structure ) ) {
			return;
		}

		if ( empty( $callback ) ) {
			$callback = array( $this, 'is_tab_visible' );
		}

		if ( is_callable( $callback ) ) {
			$result = call_user_func( $callback, $slug );
			if ( false !== $result ) {
				$this->settings_structure[ $slug ] = array(
					'title' => $title,
					'link'  => $slug,
				);
			}
	...
	public function is_tab_visible( $slug ) {
		if ( 'licenses' === $slug ) {
	...
	public function add_subtab( $tab, $data ) {
	...
	public function register_settings() {
		$this->add_tab( __( 'General', 'ultimate-member' ), 'general' );
		$this->add_tab( __( 'Access', 'ultimate-member' ), 'access' );
		$this->add_tab( __( 'Emails', 'ultimate-member' ), 'email' );
		$this->add_tab( __( 'Appearance', 'ultimate-member' ), 'appearance' );
		$this->add_tab( __( 'Extensions', 'ultimate-member' ), 'extensions' );
		$this->add_tab( __( 'Licenses', 'ultimate-member' ), 'licenses' );
		$this->add_tab( __( 'Advanced', 'ultimate-member' ), 'advanced' );
		$this->add_tab( __( 'System info', 'ultimate-member' ), 'system_info', add_query_arg( array( 'tab' => 'debug' ), admin_url( 'site-health.php' ) ) );
	}
	...
	public function init_variables() {
		do_action( 'um_settings' );

		var_dump( $this->settings_structure );
...
<p>' . __( 'If the update was successful, you will see a corresponding message. Otherwise, contact technical support if the update failed.', 'ultimate-member' ) . '</p>';
		...
		/**
		 * Filters the Ultimate Member settings fields.
		 *
		 * @param {array} $settings_structure Ultimate Member Settings configuration.
		 *
		 * @return {array} Ultimate Member Settings.
		 *
		 * @since 2.0.0
		 * @hook um_settings_structure
		 *
		 * @example <caption>Extend UM Settings with custom setting.</caption>
		 * function my_custom_um_settings_structure( $settings_structure ) {
		 *     $settings_structure['general']['sections']['pages']['fields'][] = array(
		 *         'id'    => 'setting-key',
		 *         'type'  => 'checkbox',
		 *         'label' => __( 'Setting title', 'ultimate-member' ),
		 *     );
		 *     return $settings_structure;
		 * }
		 * add_filter( 'um_settings_structure', 'my_custom_um_settings_structure' );
		 */
		$this->settings_structure = apply_filters( 'um_settings_structure', $settings_structure );
