		...
		<ul>
			...
			foreach ( UM()->account()->tabs as $id => $info ) {
				...
				if ( isset( $info['custom'] ) || ! empty( $tab_enabled ) || 'general' === $id ) {
					$current = UM()->account()->current_tab === $id ? 'tertiary-color' : 'tertiary-gray';
					?>
					<li>
						<?php echo UM()->frontend()::layouts()::button( $info['title'], array( 'type' => 'link', 'design' => $current, 'url' => UM()->account()->tab_link( $id ), 'classes' => array( 'um-link', 'um-account-link' ), 'data' => array( 'tab' => $id ), ) ); ?>
					</li>
					<?php
				}
			}
