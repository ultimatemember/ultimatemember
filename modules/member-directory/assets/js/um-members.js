...
function init_more_details_modal() {
	jQuery('.um-member-more-details-link').umModal({
		header: wp.i18n.__( 'More details', 'ultimate-member' ),
		content: function( event, options ) {
			let $modal = this;
			let $btn = options.relatedButton;
			let directory = $btn.parents('.um-directory');
			let hash = um_members_get_hash( directory );

			wp.ajax.send( 'um_member_directory_get_more_details', {
				data: {
