...
class UM_Uploader {
	...
	public function upload_image( $uploadedfile, $user_id = null, $field_key = '', $upload_type = 'stream_photo' ) {
		...
		$field_data = $ultimatemember->fields->get_field( $field_key );
		...
		$image_compression = um_get_option('image_compression');
	...
	public function validate_image_data( $file, $field_key ) {
		...
		$data = $ultimatemember->fields->get_field( $field_key );
	...
	public function profile_photo( $image_path, $src, $key, $user_id, $coord, $crop ){
		...
		$quality = um_get_option( 'image_compression' );
	...
	function remove_unused_uploads( $user_id, $new_files ) {
		...
		$user_meta_keys = $ultimatemember->user->profile;

		$_array = array();
		foreach ( $ultimatemember->builtin->custom_fields as $_field ) {
